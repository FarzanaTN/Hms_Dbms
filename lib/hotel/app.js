...
app.post('/addCustomer', (req, res) => {
    ...
    db.query(findCustomerQuery, [email, phone], (err, customerResult) => {
        ...
        if (customerResult.length > 0) {
            ...
            // Insert new customer
            const customerQuery = `
                INSERT INTO Customer (first_name, last_name, gender, email, phone)
                VALUES (?, ?, ?, ?, ?, ?)
            `;
            db.query(customerQuery, [first_name, last_name, gender, email, phone], (err, insertResult) => {
                if (err) {
                    console.error(err);
    ...
    try {
        ...
        for (const [index, reservation] of reservations.entries()) {
            ...
            try {
                ...
                // Step 1: Make all rooms available if their reservation checkout date has passed
                await new Promise((resolve, reject) => {
                    const updateAvailableRoomsQuery = `
                        UPDATE Room
                        SET available = 'yes'
                        WHERE room_id IN (
                    ...
                    `;
                    db.query(updateAvailableRoomsQuery, (err) => {
                        if (err) {
                            console.error("Error updating available rooms:", err);
                            reject("Failed to update available rooms.");
                        } else {
                            console.log("Rooms with past reservations are now available.");
                            resolve();
                        }
                    });
                });

                // Step 2: Find an available room after updating
...
app.get('/high-budget-employees', (req, res) => {
    const sql = `
        SELECT emp_id, first_name, last_name, dept_name
        FROM Employee
        WHERE dept_name IN (
            SELECT name FROM Department
            WHERE budget > ANY (SELECT budget FROM Department)
        );
    `;
    ...
    db.query(sql, (err, result) => {
        ...
        console.log(result); // Log result to check output
...
app.get('/customer-reservations', (req, res) => {
    const sql = `
...
app.get('/employees-above-avg-salary', (req, res) => {
    const sql = `
        WITH AvgSalary AS (
            SELECT dept_name, AVG(salary) AS avg_sal FROM Employee GROUP BY dept_name
        )
        SELECT e.emp_id, e.first_name, e.last_name, CAST(e.salary AS DECIMAL(10,2)) AS salary, e.dept_name
        FROM Employee e
    ...
    db.query(sql, (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).send('Error fetching employees above average salary');
        }
        res.status(200).json(result);
    });
});
...
app.get("/gpUser", (req, res) => {
    const sql = `SELECT emp_id, CONCAT(first_name, ' ', last_name) AS employee_name, phone FROM Employee WHERE phone LIKE '017%'`;
...
// API to get Employees who are NOT Security Guards
app.get("/nonSecurityEmployees", (req, res) => {
    const sql = `
      SELECT emp_id, CONCAT(first_name, " " , last_name) AS employee_name FROM Employee 
      WHERE emp_id NOT IN (SELECT emp_id FROM Security_Guard);
    `;
